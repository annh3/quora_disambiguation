...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        while (len(sentence1) < max_length):
            ...
            labels1.append(zero_label)
            mask.append(False)
        ret.append((sentence1, labels1, mask))
        ### END YOUR CODE ###
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.nn.embedding_lookup(params=embedding_tensor, ids=self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        if self.config.cell == "rnn":
            ...
            cell = tf.tf.nn.rnn_cell.LSTMCell(Config.hidden_size)
        else:
        ...
        U = tf.get_variable('U', [self.config.hidden_size, self.config.n_classes], initializer=xavier)
        W = tf.get_variable('W', [self.config.hidden_size, self.config.n_classes], initializer=xavier)
        b1 = tf.get_variable("b1",  initializer=np.array(np.zeros(self.config.n_classes), dtype=np.float32))
        b2 = tf.get_variable("b2",  initializer=np.array(np.zeros(self.config.n_classes), dtype=np.float32))
        h = tf.sigmoid(tf.matmul(state, U) + b1)
        preds = tf.nn.softmax(tf.matmul(h, W) + b2)
        return preds
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            ...
            for time_step in range(self.max_length):
                ...
                #print tf.shape(x[:, time_step,:])
                y_var, ht_var = cell(x[:,time_step,:], h)
                y_var_drop = tf.nn.dropout(y_var, self.dropout_placeholder)
                y_t = tf.matmul(y_var_drop, U) + b2
                predlist.append(y_t)
                ### END YOUR CODE

        # Make sure to reshape @preds here.
        ### YOUR CODE HERE (~2-4 lines)
        preds = tf.pack(predlist)
