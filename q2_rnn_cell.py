...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable('W_x', [self.input_size, self.state_size], initializer=xavier)
            W_h = tf.get_variable('W_h', [self.state_size, self.state_size], initializer=xavier)
            b = tf.get_variable("b",  initializer=np.array(np.zeros(self.state_size), dtype=np.float32))
