...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        #print("wow1")
        term2 = tf.matmul(state2[1], W_h) # batch size, hidden size
        #print ("wow2")
        ...
        alpha = tf.nn.softmax(tf.reshape(tf.matmul(tf.reshape(M, [-1, self.config.hidden_size]), w), [-1, self.config.max_len]))
        # batch size, max len
        #print("wow4")

        #newY = tf.transpose(output1, [0,2,1]) # batch size, hidden size, max_len
        #newY = tf.reshape(newY, [-1, self.config.max_len])
        #batch * hidden, maxen
        #print("wow5")
        r = tf.batch_matmul(tf.expand_dims(alpha, 1), output1)
        r = tf.squeeze(r)
        #print("wow6")
        ...
        self.regularizer = tf.nn.l2_loss(W_y) + tf.nn.l2_loss(W_h) + tf.nn.l2_loss(W_p) + tf.nn.l2_loss(W_x) + tf.nn.l2_loss(W_f)
        ### END YOUR CODE
    ...
    def run_epoch(self, sess, train_examples1, dev_set1, train_examples2, dev_set2, train_sizes, dev_sizes):
        ...
        for i, (obj, train_y) in enumerate(parser.minibatches(train_examples1, train_examples2, train_sizes, self.config.batch_size)):
...
def main(debug=False):
    ...
    config = Config()
    print "hello 1"
    embeddings, train_examples1, dev_set1, test_set1, train_examples2, dev_set2, test_set2, train_sizes, dev_sizes, test_sizes = parser.load_and_preprocess_data(True, config.max_len)
    # print "hello 2"
    # print "embeddings"
