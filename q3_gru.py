...
class SequencePredictor(Model):
    def add_placeholders(self):
        ...
        """
        self.inputs_placeholder = tf.placeholder(tf.float32, shape=(None, self.config.max_length, self.config.word_length), name="x")
    ...
    def add_prediction_op(self): 
        ...
        preds, state = tf.nn.dynamic_rnn(cell, self.inputs_placeholder,dtype = tf.float32)
...
def generate_sequence(max_length=20, n_samples=9999):
    ...
    for _ in range(int(n_samples/2)):
        seqs.append(([[0.,0., 0.]] + ([[0., 0., 0.]] * (max_length-1)), [0.]))
        seqs.append(([[1.,0., 0.]] + ([[0.,0., 0.]] * (max_length-1)), [1.]))
    return seqs
...
def make_dynamics_plot(args, x, h, ht_lstm, params):
...
def compute_cell_dynamics(args):
    with tf.Graph().as_default():
        ...
        with tf.variable_scope("dynamics"):
            x_placeholder = tf.placeholder(tf.float32, shape=(None,self.config.word_length))
            h_placeholder = tf.placeholder(tf.float32, shape=(None,self.config.hidden_size))
            ...
            #y_lstm, h_lstm = tf.nn.rnn_cell.LSTMCell(self.config.hidden_size)
            #y_lstm, h_lstm = tf.nn.rnn_cell.LSTMCell(self.config.hidden_size)
            y_lstm, h_lstm = tf.nn.rnn_cell.LSTMCell(self.config.hidden_size)(x_placeholder, h_placeholder, scope = "cell")
            ...
            with tf.Session() as session:
                ...
                ht_lstm = session.run([h_lstm], feed_dict={x_placeholder: x, h_placeholder: h})
                #ht_gru = np.array(ht_gru)[0]
                #ht_rnn = np.array(ht_rnn)[0]
                ht_lstm = np.array(ht_lstm)[0]
                make_dynamics_plot(args, 0, h, ht_lstm, params)
                ...
                ht_lstm = np.array(ht_lstm)[0]
                #ht_rnn = np.array(ht_rnn)[0]
                make_dynamics_plot(args, 1, h, ht_lstm, params)
